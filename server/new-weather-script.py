...
#  magic Svg template
#
template = 'weather-script-preprocess_2.svg'
template = 'weather-script-preprocess_3.svg'
...
output = output.replace('LOW_ONE',str(feels[0])).replace('LOW_TWO',str(feels[1])).replace('LOW_THREE',str(feels[2])).replace('LOW_FOUR',str(feels[3]))

# Insert current time (thanks Jennifer http://www.shatteredhaven.com/2012/11/1347365-kindle-weather-display.html)
...
output = output.replace('DAY_THREE',days_of_week[(today_dt + 2*one_day).weekday()]).replace('DAY_FOUR',days_of_week[(today_dt + 3*one_day).weekday()])
